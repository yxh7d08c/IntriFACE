...
class Xception(nn.Module):
    ...
    def __init__(self, xception_config):
        ...
        self.classifier_mode = 'similarity'
        self.id_classifier = nn.Linear(final_channel * 2, 1)
    ...
    def getEmbeddings(self, features):
        if self.mode == 'adjust_channel':
            x = features
        else:
    ...
    def classifier(self, residual, id_feat=None):
        if id_feat != None:
            out = self.last_linear(residual - id_feat)
        else:
            out = self.last_linear(residual)
        return out

    def forward(self, input):
        features = self.features(input)
        x = self.getEmbeddings(features)
