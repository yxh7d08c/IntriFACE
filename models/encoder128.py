...
def load_custom_state_dict(model, state_dict):
    ...
    for k, v in state_dict.items():
        if 'output_layer128' in k:
            if '.0.' in k:
                new_k = k.replace('output_layer128.0', 'batch_norm2d')
                new_state_dict[new_k] = v
            elif '.3.' in k:
                new_k = k.replace('output_layer128.3', 'linear')
                new_state_dict[new_k] = v
            elif '.4.' in k:
                new_k = k.replace('output_layer128.4', 'batch_norm1d')
                new_state_dict[new_k] = v
        new_state_dict[k] = v
