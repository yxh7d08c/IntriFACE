...
class IIB(nn.Module):
    ...
    def forward(self, model, B, N, readout_feats, param_dict=None):
        ...
        """
        Info = 0.
        actual_batch_size = readout_feats[0].size(0)
        X_id_restrict = torch.zeros([actual_batch_size, 512]).to(self.device)
        ...
        for i in range(N):  # multi-average information bottleneck
            ...
            Z, lambda_, info = getattr(self, f'iba_{i}')(R, readout_feats,
