...
def generate_dataset_file(dataset_name, dataset_root_path, output_file_path, compression_level='c23', perturbation = 'end_to_end'):
    ...
    ## Note: videos in Celeb-DF-v1/2 are not in the same format as in FaceForensics++ dataset
        ...
        for folder in os.scandir(dataset_path):
            if not os.path.isdir(folder):
                continue
            if folder.name == 'YouTube-real':
                continue
            if folder.name in ['Celeb-real']:
    ...
    ## Note: videos in Celeb-DF-v1/2 are not in the same format as in FaceForensics++ dataset
        ...
        dataset_dict[dataset_name] = {}
        for folder in os.scandir(dataset_path):
            if folder.name in ['Celeb-real', 'Celeb-synthesis']:
                print(folder)
                if not os.path.isdir(folder):
                    continue
                label = 'CelebDFv2_real' if folder.name == 'Celeb-real' else 'CelebDFv2_fake'
    ...
    ## DFDC dataset
        ...
        def _collect_frame_paths(root_dir, vidname):
            paths = glob.glob(os.path.join(root_dir, vidname, '*png'))
            if len(paths) == 0:
                paths = glob.glob(os.path.join(root_dir, vidname, '*jpg'))
            return paths
        ...
        for folder in os.scandir(dataset_path):
            ...
            if folder.name in ['test']:
                labels_csv = os.path.join(dataset_path, folder.name, 'labels.csv')
                if os.path.exists(labels_csv):
                    ...
                    for _, row in df.iterrows():
                        ...
                        frame_root = os.path.join(dataset_path, folder.name, 'frames')
                        frame_paths = _collect_frame_paths(frame_root, vidname)
                        if len(frame_paths) == 0:
                            continue
                        dataset_dict[dataset_name][label]['test'][vidname] = {'label': label, 'frames': frame_paths}
                        dataset_dict[dataset_name][label]['val'][vidname] = {'label': label, 'frames': frame_paths}
                    ...
                    meta_candidates = [
                        os.path.join(dataset_path, folder.name, 'metadata.json'),
                        os.path.join(dataset_path, 'metadata.json'),
                    ]
                    meta_path = next((p for p in meta_candidates if os.path.exists(p)), None)

                    with open(meta_path, 'r') as f:
                        meta = json.load(f)  # {"xxx.mp4": {"is_fake": 0/1, ...}, ...}

                    for fname, info in meta.items():
                        if not fname.endswith('.mp4'):
                            continue
                        vidname = os.path.splitext(fname)[0]
                        is_fake = info.get('is_fake')
                        if is_fake is None:
                            continue
                        label = 'DFDC_Fake' if int(is_fake) == 1 else 'DFDC_Real'
                        frame_root = os.path.join(dataset_path, folder.name, 'frames')
                        frame_paths = _collect_frame_paths(frame_root, vidname)
                        if len(frame_paths) == 0:
                            continue
                ...
                for dfdc_train_part in os.scandir(os.path.join(dataset_path, folder.name)):
                    ...
                    for video_path in os.scandir(os.path.join(dfdc_train_part, 'frames')):
                        if video_path.is_dir():
                            video_name = video_path.name
                            raw = metadata.get(video_name + '.mp4', {})
                            raw_label = raw.get('label', '')
                            assert raw_label in ['REAL', 'FAKE'], 'Invalid label: {}'.format(raw_label)
                            label = 'DFDC_Real' if raw_label == 'REAL' else 'DFDC_Fake'
                            frame_paths = [os.path.join(video_path, frame.name) for frame in os.scandir(video_path)]
                            if len(frame_paths) == 0:
                                continue
                            dataset_dict[dataset_name][label]['train'][video_name] = {'label': label, 'frames': frame_paths}
