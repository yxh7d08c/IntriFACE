...
class DeepfakeAbstractBaseDataset(data.Dataset):
    ...
    def __init__(self, config=None, mode='train'):
        ...
        if mode == 'train':
            ...
            # The training data should be collected together
    ...
    def collect_img_and_label_for_one_dataset(self, dataset_name: str):
        ...
        Returns:
            list: Image path list
            list: Label list
        
        Raises:
            ValueError: If the image path or label does not exist
            NotImplementedError: If the dataset is not implemented
        """
        # Initialize the label and frame path list
        label_list = []
        frame_path_list = []
        
        # Record the video name for video-level metrics
        video_name_list = []
        ...
        # If the JSON file exists, execute the following data collection
        ...
        for label in dataset_info[dataset_name]:
            ...
            # Traverse the videos in the dataset
            for video_name, video_info in sub_dataset_info.items():
                """
                {
                    video_name: "00000": 
                    video_info: {"label": "CelebDFv2_real", "frames": [....]}
                    ···
                }
                """
                # print("--------------------------------")
                ...
                # Get the label and frame path of the current video
                ...
                # Get the corresponding numeric label: 0 or 1 based on the text label: CelebDFv2_real or CelebDFv2_fake
                label = self.config['label_dict'][video_info['label']]
                frame_paths = video_info['frames']
                # Skip the data items without frames to avoid index errors
                ...
                # Filter the existing frames (now the data set below is 32 frames, so it will not usually execute the following block)
                total_frames = len(frame_paths)
                if self.frame_num < total_frames:
                    # If the current number of frames is greater than the required, then perform the strategy filter
                    ...
                    if self.video_level:
                        ...
                        # Calculate the total number of frames, then the step length can just take 32 frames
        ...
        if not self.lmdb:
            ...
            with self.env.begin(write=False) as txn:
                ...
                if image_bin is None:
                    ...
                    image_buf = np.frombuffer(image_bin, dtype=np.uint8)
                    # cv2.IMREAD_GRAYSCALE is grayscale, cv2.IMREAD_COLOR is color
    ...
    def __getitem__(self, index, no_norm=False):
        ...
        for image_path in image_paths:
            ...
            try:
                ...
                print(f"abstract_dataset: Error loading image at index {index}: {e}")
