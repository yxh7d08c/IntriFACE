...
def get_test_metrics(y_pred, y_true, img_names, pos_label=1):
    def get_video_metrics(image, score, label, pos_for_roc):
        result_dict = {}
        new_label = []
        new_score = []
        for item in np.transpose(np.stack((image, score, label)), (1, 0)):
        ...
        for video in image_arr:
            ...
            leng = 0
            for frame in video:
                score_sum += float(frame[1])
                label_sum += int(frame[2])
                leng += 1
            new_score.append(score_sum / leng)
            new_label.append(int(label_sum / leng))
        fpr, tpr, thresholds = metrics.roc_curve(new_label, new_score, pos_label=pos_for_roc)
    ...
    if pos_label == 1:
        ...
        pos_for_roc = 0
        ap_labels = 1 - y_true 
    fpr, tpr, thresholds = metrics.roc_curve(y_true, y_score, pos_label=pos_for_roc)
    auc = metrics.auc(fpr, tpr)
    # eer
    fnr = 1 - tpr
    eer = fpr[np.nanargmin(np.absolute((fnr - fpr)))]
    # ap
    ap = metrics.average_precision_score(ap_labels, y_score)
    # acc
    ...
    if type(img_names[0]) is not list:
        ...
        v_auc, _ = get_video_metrics(img_names, y_score, y_true, pos_for_roc)
    else:
        # video-level methods
        v_auc=auc
